...
```bash
cd /Users/user/Genealogy && git add -A && git commit -m "feat(collab): add currentRole to TreeContext and layout"
```
...
```bash
cd /Users/user/Genealogy && npx vitest run src/server-actions/__tests__/members.test.ts 2>&1 | tail -10
```
...
```bash
cd /Users/user/Genealogy && git add src/lib/supabase/admin.ts src/server-actions/__tests__/members.test.ts && git commit -m "test(collab): failing tests for members server actions"
```
...
```bash
cd /Users/user/Genealogy && npx vitest run src/server-actions/__tests__/members.test.ts 2>&1 | tail -15
```
...
```bash
cd /Users/user/Genealogy && git add src/server-actions/members.ts && git commit -m "feat(collab): members server actions (invite, list, update, remove)"
```
...
```bash
cd /Users/user/Genealogy && npx vitest run src/components/members/__tests__/MembersModal.test.tsx 2>&1 | tail -10
```
...
```bash
cd /Users/user/Genealogy && npx vitest run src/components/members/__tests__/MembersModal.test.tsx
```
...
```bash
cd /Users/user/Genealogy && git add src/components/members/ && git commit -m "feat(collab): MembersModal with invite, role change, remove"
```
...
```bash
cd /Users/user/Genealogy && git add -A && git commit -m "feat(collab): wire MembersModal + role-based UI gating"
```
...
```bash
cd /Users/user/Genealogy && git add src/app/auth/callback/route.ts && git commit -m "feat(collab): create TreeMember on invite acceptance in auth callback"
```
...
If there were fixes:
```bash
cd /Users/user/Genealogy && git add -A && git commit -m "fix(collab): resolve build/type issues"
```
